...
{
   "start" : <unix_timestamp:int64>,         # 该告警产生的时间戳
   "metric" : <metric:struct>,               # 该告警所使用的指标项配置
   "policy" : <policy:struct>,               # 触发该告警的策略 或 触发该告警的微调所属策略
   "adjust" : <adjust:struct>,               # 触发该告警的微调策略(可能为空)
   "trigger" : <trigger:struct>,             # 触发该告警的告警触发器
   "active" : <active:boolean>,              # 该告警是否仍未恢复
   "value" : <value:float64>,                # 该告警对象的当前值(仅在 Active 状态下有效)
   "notify_times" : <notify_times:int>,      # 在此次回调之前，该告警已通知过的次数(即 0 表示此次为首次通知)
   "notes" : {                               # 该告警的备注信息（可能为空）
...
其中 `<metric:struct>` 为该告警所使用的指标项配置信息，内容为：

```
{
    "name" : <name:string>,                 # 指标项 ID
    "desc" : <desc:string>,                 # 指标项描述
    "display_name" : <display_name:string>, # 人肉可读的指标项名称
    "category" : <category:string>          # 指标项的分类
}
```
...
其中 `<trigger:struct>` 为触发该告警的告警条件，内容为：

```
{
    "id" : <id:string>,                      # 此告警触发器的名称（即 ID）
    "for" : <for:int>,                       # 此告警触发器要求连续满足阈值的持续时长
    "nodata_for" : <notdata_for:int>,        # 此告警触发器要求连续满足无数据上报的持续时长
    "operator" : <operator:string>,          # 比较符，</>/<=/>=/==/!=
    "level" : <level:string>,                # 告警级别
    "threshold" : <threshold:float64>        # 告警阈值
}
```
